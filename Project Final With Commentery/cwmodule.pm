...
sub GetSummary
{
	...
	if($sth->execute)
	{
		...
		{
			my $link = "http://student.cryst.bbk.ac.uk/cgi-bin/cgiwrap/pm001/detail2.pl?accession=$accession";
...
sub GetAA
{
	...
	my $result = "<p>$AA</p><p1>Amino Acid Sequence\n</p1><p>$CDS</p>";
	return $result;
}
...
sub StickyEnds
{
	...
	my $type = $ARG[1];
	my %stickyEndsHash;
	$stickyEndsHash{EcoRI}="GAATTC";
	$stickyEndsHash{BamHI}="GGATCC";
	$stickyEndsHash{BsuMI}="CTCGAG";
